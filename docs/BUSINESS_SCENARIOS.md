...
| [原型评审后调整需求或交互](#3-原型评审后调整需求或交互) | 已生成 HTML 原型，评审后有反馈 | `/forge-html` |
...
AI 会话：描述业务需求 → ForgeTeam 自动路由
  ...
  → 涉及页面或交互：/forge-html → 用户确认 → /forge-plan
```
...
```text
AI 会话：提交原型反馈
  → 布局或样式调整：/forge-html → 用户再次确认
  → 功能或交互变更：/forge-html（同步更新 proposal.md，并重新生成原型）
                    → 用户再次确认
  → 目标、用户或核心流程重大变化：/forge-propose → 用户确认
                    → 仍涉及 UI/交互：/forge-html → 用户确认 → /forge-plan
                    → 不涉及 UI/交互：/forge-plan
```
